...
\begin{df}
Пусть $ \I $ --- это множество (любое, возможно несчётное), и для каждого $ i \in \I $ сопоставлено число $ a_i \geqslant 0 $. Определим
\begin{align*}
...
\begin{proof}\
 \begin{itemize}
     \item Пусть $ \sum_{i\in I} a_i < \infty $. Тогда для любого $ N \in \N $ существует лишь конечное число $ i \in I $ таких, что $ a_i \geqslant 1/N $(иначе из них можно составить множество с неограниченной суммой). Значит,
 \begin{align*}
  E = \left\{ i \in I \mid a_i > 0 \right\} = \bigcup_{N \in \N} \left\{ i \in I \mid a_i \geqslant 1/N \right\}
...
\item Есть тривиальное неравенство:
 ...
 .\end{align*} Потому что можно рассмотреть наборы вида $ F = \{i_1, i_2, \ldots, i_N\} $. Левая часть --- это супремум по таким $ F $ (супремум по частичным суммам) , а правая часть --- супремум по всем подмножествам $ I $, в число которых входят все частичные суммы.
 ...
 \begin{align*}
     \sup_F \sum_{i \in F} a_i \leqslant \sum_{k = 1}^{\infty} a_{i_k}
 .\end{align*}
 \end{itemize}
\end{proof}
\begin{crly}
...
\begin{df}
 ...
 \begin{itemize}
  \item $ \mu ([0, 1)) = 1 $
  \item $ \displaystyle \mu\left( \bigsqcup_{k=1}^\infty A_k \right) = \sum_{k=1}^\infty \mu( A_k ) \ \ \forall { A_k } : {k \in \mathbb{N}} \land {A_k \subset \mathbb{R}} $
...
\begin{proof}\
 \begin{enumerate}
  ...
  \item $$
   ...
   \end{cases}
   $$
   В обоих случаях имеем ячейку. То есть $[a_1, b_1) \cap [a_2, b_2) \in \mathcal{P}_1 $.
...
\begin{proof}
 ...
 .\end{align*} Пересечение не по пустому множеству, так как $ E \subset 2^X $ и  $ 2^X $ --- $ \sigma $-алгебра.
\end{proof}
\begin{df}
 ...
 \begin{enumerate}
  ...
  \item Снова индукция:
   \begin{align*}
    ...
    &P_1 \cup P_2 \cup \dots \cup P_N = \underbrace{(\bigcup_{k = 1}^{N - 1} P_k)}_{\text{ по инд. }} \sqcup \underbrace{(P_N \setminus (\bigcup_{k = 1}^{N - 1} P_k))}_{\text{ знаем из пункта 1  } }
   .\end{align*}
 \end{enumerate}
\end{proof}
\begin{crly}
...
\end{crly}
\begin{proof}
 По лемме: $ P \setminus \bigsqcup_{k=1}^N P_k = \bigsqcup_{j=1}^M Q_j $. Тогда:
 \begin{align*}
   P = \bigsqcup_{k=1}^N P_k \sqcup \bigsqcup_{j=1}^M Q_j \\
   \mu(P) = \sum \mu(P_k) + \sum \mu(Q_j) \geqslant \sum \mu(P_k)
 \end{align*}
 Переход ко второй строке совершён благодаря конечной аддитивности $ \mu $.
\end{proof}
\begin{crly}
...
.\end{align*} Теперь надо доказать неравенство в обратную сторону. Для любого $ \varepsilon > 0 $ положим $ \tilde a_{i_k} = a_{i_k} - \varepsilon/2^k $ и $ \tilde b_{i_k} = b_{i_k} $, где $ \left\{ [a_{i_k}, b_{i_k}) \right\}_{k \in \mathbb{N}}  $ --- как-то занумерованное семейство $ \left\{ [a_i, b_i) \right\}_{i \in I}  $. Тогда для любого $ t $: $ a < t < b $:
 \begin{align*}
 ...
 \begin{align*}
  [a, t) \subset \bigcup_{i_k \in F} [\tilde a_{i_k}, \tilde b_{i_k})
 \end{align*} (поменяли скобки). По предыдущему следствию:
 \begin{align*}
  t - a = &\mu([a, t)) \leqslant \sum_{i_k \in F}^{} \mu([\tilde a_{i_k}, \tilde b_{i_k})) \leqslant \sum_{i_k \in I}^{} \mu([\tilde a_{i_k}, \tilde b_{i_k})) \leqslant \sum_{i_k \in I}^{} (b_{i_k} - a_{i_k}) + \sum_{k \in \N} \frac{\varepsilon}{2^k} \leqslant \\
