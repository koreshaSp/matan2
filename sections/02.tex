...
\begin{proof}\
 \begin{enumerate}
  ...
  \item Пусть есть множество $ E \subset \bigcup_{k=1}^{\infty} E_k \subset 2^X$.
   ...
   Если существует $ k_0 $ такое, что $ \mu^\ast(E_{k_0}) = \infty $, то доказывать нечего. Если же $ \mu^\ast(E_k) < \infty $ для любого $ k \in \N $, то тогда мы можем выбрать $ A_{kj} \in S $ такие, что $ \bigcup_j A_{kj} \supset E_k $ и $$ \mu^\ast(E_k) + \frac{\varepsilon}{2^k} \geqslant \sum_{j=1}^{\infty} \mu_0(A_{kj}) $$
   Теперь возьмём и просуммируем:
   \begin{align*}
	   \mu^\ast\left( \bigcup_{k=1}^{\infty} E_k \right) &\leqslant \sum_{k,j} \mu_0(A_{k,j}) \leqslant \sum_{k=1}^{\infty} \sum_{j=1}^{\infty} \mu_0(A_{kj}) \leqslant \\ &\leqslant \sum_{k=1}^{\infty} \left(\mu^\ast(E_k) + \frac{\varepsilon}{2^k} \right) \leqslant \varepsilon + \sum_{k=1}^{\infty} \mu^\ast(E_k)
