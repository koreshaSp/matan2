...
\begin{proof}\
 \begin{enumerate}
  \item Пусть $ P \in \p $. Чтобы показать, что $ P \in \A^\ast $, необходимо и достаточно проверить, что для каждого $ E \subset X $:
   ...
   .\end{align*} Напишем
   \begin{flalign*}
	   \mu^\ast(E) & \leqslant \mua(E \cap P) + \mua(E \cap P^c) \leqslant \sum_{k=1}^\infty \mu_0(P_k \cap P) + \sum_{k=1}^\infty \sum_{j=1}^{N_k} \mu_0(Q_{kj})(\text{тк $E \subset \bigcup P_k$ и $\bigsqcup Q_{kj} = E \setminus P$}) &\\
				   & \leqslant \sum_{k=1}^\infty \mua_0(P_k) (\text{т.к. $\bigsqcup_{j=1}^{N_k} \left(Q_{kj} \sqcup (P_k \cap P)\right) \subset P_k$}) \leqslant \mu^\ast(E) + \varepsilon (\text{по неравенству выше})
   .\end{flalign*}
