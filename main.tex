...
\begin{proof}\
 \begin{itemize}
  \item Пусть $ \sum_I a_i < \infty $. Тогда для любого $ N \in \N $ существует лишь конечное число $ i \in I $ таких, что $ a_i \geqslant 1/N $. Значит,
 \begin{align*}
  \left\{ i \in I \mid a_i > 0 \right\} = \bigcup_{N \in \N} \left\{ i \in I \mid a_i \geqslant 1/N \right\} \subset E
 .\end{align*} Получается, множество $ E $ не более, чем счётное (как счётное объединение не более, чем счётных множеств).
\item Есть тривиальное неравенство:
 \begin{align*}
  \sum_{k \in N} a_{i_k} \leqslant \sum_{i \in I} a_i
 .\end{align*} Потому что можно рассмотреть наборы вида $ F = \{i_1, i_2, \ldots, i_N\} $. Левая часть --- это супремум по таким $ F $, а правая часть --- супремум по всем $ F $. Теперь докажем в обратную сторону. Пусть $ F \subset I $ конечноe. Тогда 
 \begin{align*}
...
\end{proof}
\begin{crly}
 Пусть $ I $ счётное множество, $ \{i_k\}_{k \in N} = \{i\}_{\varphi(k)} = I$ --- его нумерация двумя разными способами ($ \varphi \colon\; \N \to \N $ --- биекция). Тогда
 \begin{align*}
...
\begin{df}
 ...
 \begin{itemize}
  \item $ \mu ([0, 1)) = 1 $
  \item $ \displaystyle \mu\left( \bigsqcup_{k=1}^\infty A_k \right) = \sum_{k=1}^\infty \mu( A_k ) $
...
\begin{proof}
 ...
 .\end{align*} По аксиомам (нужно ещё вывести монотонность по включению, и то, что $ \mu [-1, 2) = 3 $):
 \begin{align*}
...
\end{df}
\begin{df}
 Пусть $ X $ --- множество. Набор подмножеств $ \mathcal{Q} \in 2^X $ --- \textit{алгебра} множеств, если
 \begin{enumerate}
  \item $ \varnothing \in Q $
  \item $ A_1, A_2 \in \mathcal{Q} \;\Longrightarrow\; A_1 \cap A_2 \in \mathcal{Q} $
  \item $ A \in \mathcal{A} \;\Longrightarrow\; A^c \in \mathcal{A} $ (аксиома симметричности)
 \end{enumerate}
\end{df}
\begin{df}
 Набор подмножеств $ \mathcal{A} \subset 2^X $ --- \textit{$ \sigma $-алгебра}, если
 \begin{enumerate}
  \item $ \varnothing \in \mathcal{A} $
  \item $ \left\{ A_i \right\}_{i \in I} $, $ I $ --- счётное множество, $ A_i \in \mathcal{A} $. Тогда
   \begin{align*}
...
\begin{exmpl}\
 \begin{itemize}
  ...
  \item $ X = \R^2 $, $ \mathcal{A} = \left\{ E \subset X \mid E \text{ --- ограничено или } E^c \text { --- ограничено } \right\}  $. Тогда $ \mathcal{A} $ --- алгебра:
   \begin{enumerate}
    \item $ \varnothing \in \mathcal{A} $
    \item Пусть $ A, B \in \mathcal{A} $ и одно из них ограничено. Тогда $ A \cap B $ ограничено и $ A \cap B \in \mathcal{A} $. А если $ A^c, B^c $ ограничены, тогда $ (A \cap B)^c = A^c \cup B^c $ ограничено. Тогда $ A \cap B \in \mathcal{A} $.
    \item $ A \in \mathcal{A} \;\Longleftrightarrow\; A^c \in \mathcal{A} $
   \end{enumerate}
   При этом $ \mathcal{A} $ не является $ \sigma $-алгеброй:
   \begin{align*}
    \R^2 \setminus \left\{ (0, x) \mid x \geqslant 0 \right\}  = \bigcap_{k=1}^\infty A_k, \quad A_k \in \mathcal{A} \notin \mathcal{A}
   .\end{align*} Нужно взять
   \begin{align*}
    A_k = \R^2 \setminus \left\{ (0, x) \mid 0 \leqslant x \leqslant k \right\} 
   .\end{align*}
  \item Если $ \mathcal{A} $ --- алгебра, то для любых $ A_1, \ldots, A_N \in \mathcal{A} $ верно $ \bigcup_1^N A_k \mathcal{A} $, так как
   \begin{align*}
    \bigcup_1^N A_k = \left( \bigcup_{k=1}^N A_k^c \right)^c
   .\end{align*}
  \item Аналогично, если $ \mathcal{A} $ --- $ \sigma $-алгебра, то для любого счетного $ I $ ($ A_i \in \mathcal{A} $ для любого $ i \in I $), то
   \begin{align*}
...
\end{remrk}
\begin{lm}
 Если $ \mathcal{A}_i $ --- $ \sigma $-алгебра, $ i \in I $, $ I $ --- произвольное множество. Тогда $ \mathcal{A} = \bigcap_{i \in I} \mathcal{A}_i $ --- $ \sigma $-алгебра.
\end{lm}
\begin{proof}\
 \begin{enumerate}
  \item $ \varnothing \in \mathcal{A} $, так как $ \varnothing \in \mathcal{A}_i $ для всех $ i \in I $.
  \item $ A \in \mathcal{A} \;\Longrightarrow\; A \in \mathcal{A}_i \forall i \in I \;\Longrightarrow\; A^c \in \mathcal{A}_i \forall i \in I \;\Longrightarrow\; A^c \in \bigcup_{i \in I} \mathcal{A}_i = \mathcal{A}$ 
  \item TODO
 \end{enumerate}
\end{proof}
\begin{lm}
 Если $ E \subset 2^X $, то существует единственная $ \mathcal{A}_E $ --- наименьшая по включению $ \sigma $-алгебра, содержащая $ E $.
\end{lm}
\begin{proof}
 Есть просто формула:
 \begin{align*}
  \mathcal{A}_E := \bigcap_{E \subset \mathcal{A} \text{---} \sigma \text{-алгебра}} \mathcal{A}
...
\begin{df}
 ...
 \begin{align*}
  P = \bigsqcup_{k=1}^N P_k \subset \mathcal{P}
...
\end{df}
\begin{df}
 Пусть $ \mathcal{A} $ --- $ \sigma $-алгебра в $ X $, $ \mu \colon\, \mathcal{A} \to [0, +\infty] $ --- \textit{мера}, если
 \begin{align*}
  \mu\left( \bigsqcup_{i \in I} A_i \right) = \sum_{i\in I}^{  } \mu(A_i)
 \end{align*} для любого дизъюнктного счётного набора множеств $ A_i \in \mathcal{A}$.
\end{df}
\begin{exmpl}\
 \begin{itemize}
  \item $ \mu \equiv 0 $, $ \mathcal{A} = 2^X $ (или $ \mathcal{A} $ любая) 
  \item $ \mu \equiv +\infty $, $ \mathcal{A} = 2^X $ (или $ \mathcal{A} $ любая) 
...
\begin{lm}[о подчинённом разбиении]
 ...
 \begin{enumerate}
  \item $ \displaystyle P \setminus \bigcup_{k=1}^N P_k = \bigsqcup_{j=1}^N Q_j $, где $ Q_j \in \mathcal{P} $
 ...
 \begin{enumerate}
  \item По индукции. $ N = 1$ --- аксиома 3 полукольца. Пусть
   ...
   \item
    \begin{align*}
     &P_1 = P_1 = Q_{1,1} \\
     &P_1 \cap P_2 = P_1 \sqcup (P_2 \setminus P_1) = Q_{1,1} \sqcup (Q_{2,1} \sqcup \ldots \sqcup Q_{2, M_2}) \\
...
\begin{proof}
 ...
 \begin{align*}
  ...
  \mu\left( \bigsqcup_{i \in F} [a_i, b_i] \right) \leqslant \mu([a, b)) \text { <- следствие 23 }
 .\end{align*} Теперь надо доказать неравенство в обратную сторону. Для любого $ \varepsilon > 0 $ положим $ \tilde a_{i_k} = a_{i_k} - \varepsilon/2^k $ и $ \tilde b_{i_k} = b_{i_k} $, где $ \left\{ [a_{i_k}, b_{i_k}) \right\}  $ --- как-то занумерованное семейство $ \left\{ [a_i, b_i) \right\}_{i \in I}  $. Тогда для любого $ t $: $ a < t < b $:
 \begin{align*}
  [a, t] \subset \bigcup_{k \in \N} (\tilde a_{i_k}, \tilde b_{i_k})
 .\end{align*} Но слева \textbf{компактное} множество. Тогда, существует $ F $ конечное, такое что
 \begin{align*}
  [a, t) \subset \bigcup_{i_k \in F} [\tilde a_{b_k}, \tilde b_{i_k})
 \end{align*} (поменяли скобки). По предыдущему следствию:
 \begin{align*}
  t - a = &\mu([a, t)) \leqslant \sum_{i_k \in F}^{} \mu([\tilde a_{i_k}, \tilde b_{i_k})) \leqslant \sum_{i_k \in I}^{} (b_{i_k} - a_{i_k}) + \sum_{k \in \N} \frac{\varepsilon}{2^k} \leqslant \\
  \leqslant &\sum_{i \in I} \mu([a_i, b_i)) + \varepsilon
 \end{align*} для любого $ t < b $ и $ \varepsilon > 0 $. Устремим $ t \to b $ и $ \varepsilon \to 0 $.
\end{proof}
