...
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
	\incfig[0.8]{differential_of_parallel_path_f}
...
\end{figure}
 ...
 Обозначим $h_x = (h_1, 0), \ h_y = (0, h_y)$, тогда

 \begin{align*}
	 \Gamma : [0, 2] \to \Omega, \ \Gamma(s) &= \begin{cases} 
																						 z + s h_x & 0 \leqslant s < 1 \\ 
																						 z + h_x + (s - 1) h_y & 1 \leqslant s \leqslant 2
																					 \end{cases} \\
																					 &= [\rho(s) = z + s h_x, \ \zeta(s) = z + h_x + s h_y] \\
																					 &= \begin{cases}
																							\rho(s) & 0 \leqslant s < 1 \\
																							\zeta(s - 1) & 1 \leqslant s \leqslant 2
																					 \end{cases}
 \end{align*}
...
{\allowdisplaybreaks
 \begin{align*}
	 \int_{\Gamma} \omega &= \int\limits_0^2 P(\Gamma(t))\Gamma_1'(t) + Q(\Gamma(t)) \Gamma_2'(t) \ dt \\
												&= \int\limits_0^1 P(\rho(t))\underbrace{\rho_1'(t)}_{=h_1} + Q(\rho(t)) \underbrace{\rho_2'(t)}_{=0} \ dt \ + \\ 
												&\ \ +  \int\limits_1^2 P(\zeta(t-1))\underbrace{\zeta_1'(t-1)}_{=0} + Q(\zeta(t-1)) \underbrace{\zeta_2'(t-1)}_{=h_2} \ dt \\
											  &= \int\limits_{0}^{1} P(\rho(t)) h_1 \ dt + \int\limits_0^1 Q(\zeta(t)) h_2 \ dt \\
