...
\begin{figure}[ht]
    \centering
	\incfig[0.9]{special_point_in_rectangle}
...
\begin{proof}[\normalfont\textsc{Доказательство}]
 ...
 \begin{align*}
	 ...
	 \implies g'(z) &= (g(z)) d(x + i \cdot y) = g'(z)dx + i \cdot g'(z)dy = F'_x(x,y)dx + F'_y(x,y)dy \implies \\
	 \implies F'_x(x, y) &= g'(z), F'_y(x, y) = g'(z) \cdot i
 \end{align*} Проверим, что такой выбор нам подходит: по определению дифференциала
 \begin{align}
	 (*) \ F(x,y) = F(x_0, y_0) + (x-x_0) F'_x(x_0, y_0) + (y-y_0)F'_y(x_0,y_0) + o(\sqrt{\left| x-x_0 \right|^2 + \left| y-y_0 \right|^2})
 \end{align}

 Заметим, что (*) равносильна
 \begin{align*}
