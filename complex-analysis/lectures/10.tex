...
\begin{lm}
 \label{lemma:Convergence of Functions on Compact}
 Пусть $ (X,\rho) $ --- метрической компакт, и последовательность непрерывных функций  $ \{f_{n}\}_{n=1}^{\infty} \subset C(X,\CC)$ сходится во всех точках $ x \in X $ к функции~$ f \colon X \to \CC $. Тогда эта сходимость равномерная, а также предельная функция непрерывна: $ f \in C(X,\CC) $.
\end{lm}
\begin{proof}[\normalfont\textsc{Доказательство}]
 
Шаг 1. Обоснуем, что сходимость равномерная. Хотим показать что для любого $ \eps > 0 $ и для любого $ x \in X $ существует $ N_x $:
\begin{align*}
...
\end{align*}  для любого $ y $, $ \rho(x, y) \leqslant \delta $, где $ \delta $ взята из определения равномерной непрерывности $ f_n $, так как
\begin{align*}
...
Шаг 2. Покажем что полученная функция $f$ непрерывна. Предел --- непрерывная функция: если $ \eps > 0 $, $ \delta $ --- из \eqref{eq:equicontinuity}, то
\begin{align*}
