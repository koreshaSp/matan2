...
\begin{proof}[\normalfont\textsc{Доказательство}]
 ...
 \begin{align*}
  \limsup_{z \to a} \left| (z-a)^{N}f(z) \right| < +\infty,
 \end{align*} то $ f = O(\frac{1}{(z-a)^{N}}) $, и $ a $ --- полюс.
\end{proof}

\begin{exmpl}
 У функции $ e^{1 / z} $ точка $ 0 $ --- это существенная особая точка, так как
 \begin{align*}
  \limsup_{z \to 0} \left| z \right|^{k} \left| e^{1/z} \right| \geqslant \limsup_{\eps \to 0+} \eps^{k} e^{1 / \eps} = +\infty
...
\begin{proof}[\normalfont\textsc{Доказательство}]
 ...
 \end{align*} где $ g(a) \neq 0 $. Тогда
 \begin{align*}
  \frac{f'}{f} &= \frac{N(z-a)^{N-1}g + (z-a)^{N}g'}{(z-a)^{N}g} = \frac{N}{z-a} + O(1)
...
\begin{conventn*}
 Запись $ \oint_{\partial \Omega} f \, dz $ означает, что граница $ \partial \Omega $ ориентированна так, чтобы область оставалась слева при обходе.
...
\begin{align*}
 \oint_{\partial \Omega} f\,dz = 2\pi i \sum_{a \in E} \res_{a} f.
\end{align*}
\end{thm}
\begin{proof}[\normalfont\textsc{Доказательство}]
 Доказательство будет по модулю формулы Стокса. 

 Рассмотрим область $ \tilde \Omega = \Omega \setminus \bigcup_{a \in E} B(a,\eps) $, где $ \eps > 0 $ достаточно маленькое, чтобы все круги $ B(a,\eps) $ все лежали в $ \Omega $ и не пересекались.  Тогда
 \begin{align*}
  \int\limits_{\partial \tilde \Omega}   f\,dz = \oint\limits_{\tilde \Omega} d(f\,dz) = 0.
 \end{align*} Но
 \begin{align*}
  \oint\limits_{\partial \tilde \Omega} f\,dz = \oint\limits_{\partial \Omega} f\,dz - \sum_{a \in E} \underbrace{\oint\limits_{C(a,\eps)}f\,dz}_{=2\pi i \cdot \res_a f}.
 \end{align*} Здесь
 \begin{align*}
  \oint\limits_{C(a,\eps)}f(z)\,dz = \oint\limits_{C(a,\eps)} \left( \sum_{k \in \Z} c_k(z-a)^{k} \right)dz = 2\pi i \cdot c_{-1} = 2\pi r \cdot \res_a f.
 \end{align*}
\end{proof}
...
\begin{crly}
 ...
 \begin{align*}
  f(w) = \frac{1}{2\pi i} \oint\limits_{\partial\Omega} \frac{f(z)}{z-w}\,dz,\quad w \in \Omega.
 \end{align*}
\end{crly}
\begin{proof}[\normalfont\textsc{Доказательство}]
 По теореме Коши
 \begin{align*}
  \oint\limits_{\partial \Omega} \frac{f(z)}{z-w}\,dz = 2\pi i \res_w \frac{f}{z-w} = 2\pi i f(w).
 \end{align*}
\end{proof}
...
\begin{proof}[\normalfont\textsc{Доказательство}]
 ...
 \begin{align*}
  f(w) = \frac{1}{2\pi i} \oint\limits_{\partial\Omega} \frac{f(z)}{z-w}\,dz.
 \end{align*} Аналогичная формула верна для $ g_n $. Тогда
 \begin{align}
  f'(w) = \frac{1}{2\pi i} \oint\limits_{\partial\Omega} \frac{f(z)}{(z-w)^{2}}\,dz.
 \end{align} Почему можно дифференцировать?
 \begin{align*}
  \frac{f(w+\xi)-f(w)}{\xi} = \frac{1}{2\pi i} \oint\limits_{\partial \Omega} f(z) \underbrace{\frac{1}{\xi} \left( \frac{1}{z-(w+\xi)} - \frac{1}{z - w} \right)}_{\frac{1}{(z-w-\xi)(z-w)} = O(\frac{1}{(z-w)^{2}}) = O(1)}\,dz
 \end{align*} По теореме Лебега можно переходить к пределу и получим (*). Тогда
 \begin{align*}
  \left| f'(w) - g'(w) \right| \leqslant \frac{1}{2\pi} \oint\limits_{\partial\Omega} \frac{\max_{\partial\Omega} \left| f-g_n \right|}{\left| z-w \right|^{2}}\,dz \to 0,
 \end{align*} равномерно по $ w \in K $, где $ K $ --- компакт в $ \Omega $.
\end{proof}
